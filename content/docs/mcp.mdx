---
title: MCP Server
description: Use AI agents to manage GudCal via the Model Context Protocol. 21 tools for full agent autonomy.
---

GudCal includes a built-in **Model Context Protocol (MCP)** server with **21 tools** that allow AI agents to manage scheduling — from creating event types and setting availability to confirming bookings and managing API keys.

Public source and listing metadata live in **[gudlab/gudcal-core](https://github.com/gudlab/gudcal-core)** (`server.json`, `.mcp.json`). Do not point agents at the private `cavewebs/gudcal` repo.

On **GudCal Cloud**, hosted MCP at `https://gudcal.com/api/mcp` is a **Team** plan feature ($20/mo). Self-hosting for your own org is allowed under BSL 1.1; offering a competing hosted Calendly is not.
...
The MCP server uses HTTP transport (streamable HTTP) with JSON-RPC messages.
...
Official registry metadata: [`server.json`](https://github.com/gudlab/gudcal-core/blob/main/server.json) in the public repo. Cursor / Open Plugins: [`.mcp.json`](https://github.com/gudlab/gudcal-core/blob/main/.mcp.json).
...
Cloud MCP is Team. Point `url` at `https://your-instance.example.com/api/mcp` when you self-host for your own org.

### Claude Desktop

Add GudCal to `~/Library/Application Support/Claude/claude_desktop_config.json`:
...
Self-hosted instances expose `POST /api/mcp` from this source. BSL 1.1 allows that for **your** organization. It does not allow you to productize a hosted Calendly competitor. Cloud Team gating on gudcal.com is unchanged.
